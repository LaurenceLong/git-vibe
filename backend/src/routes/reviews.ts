...
export async function reviewRoutes(server: FastifyInstance) {
  ...
  server.post<{ Params: { id: string } }>('/api/changesets/:id/reviews/threads', async (request, reply) => {
    try {
  ...
  server.post<{ Params: { id: string; threadId: string } }>(
    ...
    async (request, reply) => {
      ...
      const updated = await reviewThreadsRepository.update(request.params.threadId, {
        status: 'resolved',
      });
